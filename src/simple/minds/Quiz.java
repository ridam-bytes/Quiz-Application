...
public class Quiz extends JFrame implements ActionListener {
    ...
    public static int ans_given=0;// to check ans given bu person.
    public static int score=0; 
    JLabel qno, question;
    String q[][] = new String[10][5];  
    String pa[][] = new String[10][1];// to store person answers of quiz.
    String qa[][] = new String[10][2]; // to store correct answers of question.
    ...
    String username;
     ...
     Quiz(String username)  {
        ...
        opt2 = new JRadioButton("");
         ...
         options.add(opt1);
         options.add(opt2);  // group all check buttons so that only one option will be checked.
         options.add(opt3);
         options.add(opt4);
         ...
         submit.setEnabled(false);/// initially submit button will be disabled.
     ...
     public void actionPerformed(ActionEvent ae){
         if(ae.getSource()== next){
             repaint();//applet to make next frame same as previous frame and it internall calls paint() function.
     ...
     public void paint(Graphics g){
         super.paint(g); // to call parent class of paint function means previous frame.
         String time = "Time Left - " + timer + "seconds";
         g.setColor(Color.RED);//setColor function of applet used to change color of foreground.
         ...
         try{
             Thread.sleep(1000); /*1second*/ // use to call repaint function again.
             repaint();
     ...
     // start function kept the value of qno and settext is used to set text for label that is string but count is int so we will have to change int count to string first    
         qno.setText( count+1 + ". ");
         question.setText(q[count][0]);
         opt1.setLabel(q[count][1]);
         opt1.setActionCommand(q[count][1]); // set value in opt1
